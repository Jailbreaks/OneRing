...
static int hostapd_cli_cmd_send_nrrep(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    char cmd[256];
    int res;
    if (argc != 4) {
        printf("Invalid NRREP command - requires: <DST_MAC> <OPCLASS> <CHAN> <COUNT>\n");
        return -1;
    }

    res = os_snprintf(cmd, sizeof(cmd), "NRREP %s %s %s %s", argv[0], argv[1], argv[2], argv[3]);
    if (os_snprintf_error(sizeof(cmd), res)) {
        printf("Too long NRREP command.\n");
        return -1;
    }
    return hostapd_cli_cmd(ctrl, "NRREP", 4, argc, argv);
}

static int hostapd_cli_cmd_send_addba_req(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    char cmd[256];
    int res;
    if (argc != 3) {
        printf("Invalid ADDBA command - requires: <DST_MAC> <TID> <IMMEDIATE>\n");
        return -1;
    }

    res = os_snprintf(cmd, sizeof(cmd), "ADDBA %s %s %s", argv[0], argv[1], argv[2]);
    if (os_snprintf_error(sizeof(cmd), res)) {
        printf("Too long ADDBA command.\n");
        return -1;
    }
    return hostapd_cli_cmd(ctrl, "ADDBA", 3, argc, argv); 
}

static int hostapd_cli_cmd_send_delba(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    char cmd[256];
    int res;
    if (argc != 2) {
        printf("Invalid DELBA command - requires: <DST_MAC> <TID>\n");
        return -1;
    }

    res = os_snprintf(cmd, sizeof(cmd), "DELBA %s %s", argv[0], argv[1]);
    if (os_snprintf_error(sizeof(cmd), res)) {
        printf("Too long DELBA command.\n");
        return -1;
    }
    return hostapd_cli_cmd(ctrl, "DELBA", 2, argc, argv); 
}

static int hostapd_cli_cmd_send_specmeas_req(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    if (argc < 2) {
        printf("Invalid SPECMEAS command - requires: <DST_MAC> <FRAMEDATA> [OPTIONAL: <FRAMEDATA> ...]\n");
        return -1;
    }

    return hostapd_cli_cmd(ctrl, "SPECMEAS", 2, argc, argv); 
}

static int hostapd_cli_cmd_send_rmreq(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    if (argc < 2) {
        printf("Invalid RMREQ command - requires: <DST_MAC> <FRAMEDATA> [OPTIONAL: <FRAMEDATA> ...]\n");
        return -1;
    }

    return hostapd_cli_cmd(ctrl, "RMREQ", 2, argc, argv); 
}

static int hostapd_cli_cmd_send_rmrep(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    if (argc < 2) {
        printf("Invalid RMREP command - requires: <DST_MAC> <FRAMEDATA> [OPTIONAL: <FRAMEDATA> ...]\n");
        return -1;
    }

    return hostapd_cli_cmd(ctrl, "RMREP", 2, argc, argv); 
}

static int hostapd_cli_cmd_send_nrrep_raw(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    if (argc < 2) {
        printf("Invalid NRREP_RAW command - requires: <DST_MAC> <FRAMEDATA> [OPTIONAL: <FRAMEDATA> ...]\n");
        return -1;
    }

    return hostapd_cli_cmd(ctrl, "NRREP_RAW", 2, argc, argv); 
}

static int hostapd_cli_cmd_send_get_backdoor_state(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    if (argc != 0) {
        printf("Invalid GET_BACKDOOR_STATE command - takes no arguments\n");
        return -1;
    }

    return hostapd_cli_cmd(ctrl, "GET_BACKDOOR_STATE", 0, argc, argv); 
}

static int hostapd_cli_cmd_send_clear_backdoor_state(struct wpa_ctrl *ctrl, int argc,
                    char* argv[])
{
    if (argc != 0) {
        printf("Invalid CLEAR_BACKDOOR_STATE command - takes no arguments\n");
        return -1;
    }

    return hostapd_cli_cmd(ctrl, "CLEAR_BACKDOOR_STATE", 0, argc, argv); 
}
...
static const struct hostapd_cli_cmd hostapd_cli_commands[] = {
    ...
    { "nrrep", hostapd_cli_cmd_send_nrrep, NULL, NULL },
    { "addba", hostapd_cli_cmd_send_addba_req, NULL, NULL },
    { "delba", hostapd_cli_cmd_send_delba, NULL, NULL },
    { "specmeas", hostapd_cli_cmd_send_specmeas_req, NULL, NULL },
    { "rmreq", hostapd_cli_cmd_send_rmreq, NULL, NULL },
    { "rmrep", hostapd_cli_cmd_send_rmrep, NULL, NULL },
    { "nrrep_raw", hostapd_cli_cmd_send_nrrep_raw, NULL, NULL },
    { "get_backdoor_state", hostapd_cli_cmd_send_get_backdoor_state, NULL, NULL },
    { "clear_backdoor_state", hostapd_cli_cmd_send_clear_backdoor_state, NULL, NULL },
    { NULL, NULL, NULL, NULL }
};
